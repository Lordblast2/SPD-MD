const updateHerokuVar = async ({ Striker, m, reply, isCreator, key, value, setHerokuEnvVar }) => {
  ...
  try {
    ...
    reply("🔄 *Striker Bot will restart to apply changes!*");
  } catch (error) {
...
module.exports = [
  {
    command: ["addvar"],
    operate: async ({ Striker, m, reply, isCreator, full_args, setHerokuEnvVar }) => {
      if (!isCreator) return;
      ...
      await updateHerokuVar({ Jinwoo, m, reply, isCreator, key: varName, value: varValue, setHerokuEnvVar });
    }
  },
  {
    command: ["delvar"],
    operate: async ({ Striker, m, reply, isCreator, full_args, deleteHerokuEnvVar }) => {
      if (!isCreator) return;
      ...
      try {
        ...
        reply("🔄 *Jinwoo Bot will restart to apply changes!*");
      } catch (error) {
  ...
  {
    command: ["setbotname"],
    operate: async ({ Striker, m, reply, isCreator, text, setHerokuEnvVar }) => {
      if (!text) return reply("*Provide a bot name*\n\nExample: .setbotname Striker Alpha");
      await updateHerokuVar({ Striker, m, reply, isCreator, key: "BOT_NAME", value: text.trim(), setHerokuEnvVar });
    }
  },
    {
    command: ["setname"],
    operate: async ({ Striker, m, reply, isCreator, text, setHerokuEnvVar }) => {
      if (!text) return reply("*Provide your name*\n\nExample: .setname Malvin");
      await updateHerokuVar({ Striker, m, reply, isCreator, key: "OWNER_NAME", value: text.trim(), setHerokuEnvVar });
    }
  },
    {
    ...
    operate: async ({ Striker, m, reply, isCreator, text, setHerokuEnvVar }) => {
      ...
      await updateHerokuVar({ Striker, m, reply, isCreator, key: "OWNER_NUMBER", value: text.trim(), setHerokuEnvVar });
    }
  },
  {
    command: ['setvar'],
    operate: async (context) => {
        ...
        try {
            ...
            await reply(`*Striker Xmd will restart to apply the new environment variable. Please wait a moment!*`);
        } catch (error) {
