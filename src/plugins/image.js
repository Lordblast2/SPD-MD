...
module.exports = [
  {
    ...
    operate: async ({ m, prefix, command, Striker, mess, reply }) => {
const quoted = m.quoted ? m.quoted : null || m.msg ;
      ...
      try {
        ...
        await Jinwoo.sendMessage(m.chat, { image: enhancedImage, caption: "*Image enhanced successfully*" }, { quoted: m });
      } catch (error) {
        console.error(error);
  ...
  {
    command: ['wallpaper'],
    operate: async ({ m, text, Striker, reply }) => {
