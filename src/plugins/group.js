...
module.exports = [
 {
    ...
    operate: async (context) => {
        const { m, mess, text, isCreator, reply,Striker } = context;
        ...
        await Striker.groupParticipantsUpdate(m.chat, [bws], "add");
        reply(mess.done);
    }
 },
  {
...
},
 {
...
},
  {
    command: ['close'],
    operate: async (context) => {
        const { m, mess, isAdmins, isCreator, isBotAdmins, Striker, reply } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);

        Jinwoo.groupSettingUpdate(m.chat, "announcement");
...
},
 {
    command: ['closetime'],
    operate: async (context) => {
        const { m, mess, args, isAdmins, isCreator, isBotAdmins, Striker, reply } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['delppgroup'],
    operate: async (context) => {
        const { m, mess, isAdmins, isCreator, isBotAdmins, Striker, reply, from } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
        
        await Jinwoo.removeProfilePicture(from);
...
}, 
  {
    command: ['demote'],
    operate: async (context) => {
        const { m, mess, text, isAdmins, isGroupOwner, isCreator, isBotAdmins, Striker } = context;
        if (!m.isGroup) return reply(mess.group);
        ...
        let bwstq = m.mentionedJid[0]
            ...
            ? m.quoted.sender
            : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
        await Striker.groupParticipantsUpdate(m.chat, [bwstq], "demote");
        reply(mess.done);
    }
},
 {
    ...
    operate: async (context) => {
        const { m, mess, args, isAdmins, isGroupOwner, isCreator, isBotAdmins, Striker, prefix, command } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);

        if (args[0] === "on") {
            await Striker.groupSettingUpdate(m.chat, "unlocked").then(
                (res) => reply(`*Successful, members can edit group info*`)
            );
        } else if (args[0] === "off") {
            await Striker.groupSettingUpdate(m.chat, "locked").then((res) =>
...
}, 
 {
  ...
  operate: async ({ Striker, m, reply, isAdmins, isGroupOwner, isCreator, mess, isBotAdmins, groupMetadata }) => {
    if (!m.isGroup) return reply(mess.group);
    if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
    if (!isBotAdmins) return reply(mess.admin);

    let response = await Striker.groupInviteCode(m.chat);
    Jinwoo.sendText(
      m.chat,
...
},
 {
    command: ['hidetag', 'tag'], 
    operate: async (context) => {
        const { m, isAdmins, isGroupOwner, isCreator, mess, participants, Striker, isBotAdmins } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
  {
    command: ['invite'],
    operate: async (context) => {
        const { m, mess, text, prefix, Jinwoo, isBotAdmins } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isBotAdmins) return reply(mess.admin);
        if (!text)
            return reply(
        ...
        let group = m.chat;
        let link = "https://chat.whatsapp.com/" + (await Striker.groupInviteCode(group));
        await Jinwoo.sendMessage(text + "@s.whatsapp.net", {
...
},
  {
    ...
    operate: async (context) => {
        ...
        let bck = m.mentionedJid[0]
            ...
            ? m.quoted.sender
            : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
        await Jinwoo.groupParticipantsUpdate(m.chat, [bck], "remove");
        reply(mess.done);
    }
},
 {
  ...
  operate: async (context) => {
    const { m, mess, args, store, botNumber, Striker, reply } = context;
    if (!m.isGroup) return reply(mess.group);
    ...
    let liston = 1;
    Jinwoo.sendText(m.chat, '*ONLINE MEMBERS IN THIS GROUP*\n\n' + online.map(v => `${liston++} . @` + v.replace(/@.+/, '')).join`\n`, m, { mentions: online });
  }
},
  {
    command: ['mediatag'],
    operate: async (context) => {
        const { m, isAdmins, mess, participants, Striker, isBotAdmins } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['open'],
    operate: async (context) => {
        const { m, mess, isAdmins, isCreator, isBotAdmins, Striker } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);

        Jinwoo.groupSettingUpdate(m.chat, "not_announcement");
...
},
 {
    command: ['opentime'],
    operate: async (context) => {
        const { m, mess, args, isAdmins, isCreator, isBotAdmins, Striker } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['poll'],
    operate: async (context) => {
        const { m, mess, text, isCreator, prefix, Striker, isGroup } = context;
...
},
  {
    command: ['promote'],
    operate: async (context) => {
        const { m, mess, text, isAdmins, isGroupOwner, isCreator, isBotAdmins, quoted, Striker } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);
        ...
        let bwst = m.mentionedJid[0]
            ...
            ? m.quoted.sender
            : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
        await Striker.groupParticipantsUpdate(
            m.chat,
            [bwst],
            "promote"
        );
...
},
 {
    command: ['resetlink'],
    operate: async (context) => {
        const { m, isAdmins, isGroupOwner, isCreator, mess, Jinwoo, isBotAdmins } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);

        await Jinwoo.groupRevokeInvite(m.chat).then((res) => {
          reply(mess.done);
        });
    }
},
  {
    ...
    operate: async (context) => {
        ...
        await Jinwoo.groupUpdateDescription(m.chat, text);
        reply(mess.done);
    }
},
  {
    ...
    operate: async (context) => {
        ...
        await Jinwoo.groupUpdateSubject(m.chat, text);
        reply(mess.done);
    }
},
  {
  command: ['setppgroup'],
  operate: async ({ m, reply, mess, isAdmins, isCreator, isBotAdmins, Striker, quoted, mime, prefix, command, args }) => {
    if (!m.isGroup) return reply(mess.group);
    if (!isAdmins && !isCreator) return reply(mess.notadmin);
    if (!isBotAdmins) return reply(mess.admin);
    ...
    const medis = await Jinwoo.downloadAndSaveMediaMessage(quoted, "ppbot.jpeg");
    if (args[0] === "full") {
      ...
      await Jinwoo.updateProfilePicture(m.chat, { url: medis });
      fs.unlinkSync(medis);
...
},
{
  ...
  operate: async ({ Jinwoo, m, reply, mess, participants, groupMetadata }) => {
    if (!m.isGroup) return reply(mess.group);
...
}, 
 {
    command: ['tagall'],
    operate: async (context) => {
        const { m, isAdmins, isGroupOwner, isCreator, mess, q, participants, Jinwoo, isBotAdmins } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
  ...
  operate: async ({ Striker, m, reply, mess, participants, isGroupAdmins, isCreator, sleep, groupMetadata }) => {
    if (!m.isGroup) return reply(mess.group);
...
},
  {
    ...
    operate: async (context) => {
        const { m, mess, isCreator, Jinwoo } = context;
        if (!isCreator) return reply(mess.owner);
        if (!m.isGroup) return reply(mess.group);
 ...
 {
  command: ['vcf'],
  operate: async ({ Striker, m, reply, mess, participants, isGroupAdmins, isCreator, groupMetadata }) => {
    if (!m.isGroup) return reply(mess.group);
    if (!isGroupAdmins) return reply(mess.admin);

    let cmiggc = await Jinwoo.groupMetadata(m.chat);
    let vcard = "";
    let noPort = 0;
